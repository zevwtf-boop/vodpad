...
import { h, fmtClock } from './util.js?v=66fb115653';
import { state, card, commit, cardTitle, deleteCard, childrenOf, SEV_LABEL } from './store.js?v=66fb115653';
import { contextMenu, toast, promptDialog, confirmDialog } from './ui.js?v=66fb115653';
...
async function blockMenu(blk, at) {
  ...
  contextMenu([
    ...
    { sep: true },
    { label: 'note in the margin', icon: 'sidenote', hint: 'ctrl+m', onPick: async () => (await import('./page.js?v=66fb115653')).addSidenoteFromSelection() },
    { label: 'link this line to a picture', icon: 'link', hint: 'ctrl+l', onPick: async () => (await import('./anchors.js?v=66fb115653')).startAnchorPick() },
    { label: 'make it a sub-page', icon: 'cards', onPick: async () => (await import('./page.js?v=66fb115653')).addSubPage(id) },
    { sep: true },
    ...clipboardRows(),
    { sep: true },
...
function insertItems(afterId) {
  return [
    { label: 'picture from a file', icon: 'image', onPick: async () => (await import('./images.js?v=66fb115653')).pickImageFile(afterId) },
    { label: 'heading', icon: 'h2', onPick: async () => { const ed = await import('./editor.js?v=66fb115653'); ed.insertBlock(afterId, { type: 'h2' }, true); } },
    { label: 'bullet list', icon: 'listUl', onPick: async () => { const ed = await import('./editor.js?v=66fb115653'); ed.insertBlock(afterId, { type: 'ul' }, true); } },
    { label: 'checklist', icon: 'listCheck', onPick: async () => { const ed = await import('./editor.js?v=66fb115653'); ed.insertBlock(afterId, { type: 'todo' }, true); } },
    { label: 'quote', icon: 'quote', onPick: async () => { const ed = await import('./editor.js?v=66fb115653'); ed.insertBlock(afterId, { type: 'quote' }, true); } },
    { label: 'callout', icon: 'callout', onPick: async () => { const ed = await import('./editor.js?v=66fb115653'); ed.insertBlock(afterId, { type: 'callout' }, true); } },
    { label: 'code', icon: 'codeTag', onPick: async () => { const ed = await import('./editor.js?v=66fb115653'); ed.insertBlock(afterId, { type: 'code' }, true); } },
    { label: 'table', icon: 'table', onPick: async () => { const ed = await import('./editor.js?v=66fb115653'); const made = ed.insertBlock(afterId, { type: 'p' }, false); ed.setType(made.id, 'table', { rows: [['', '', ''], ['', '', ''], ['', '', '']], header: true }); } },
    { label: 'divider', icon: 'divider', onPick: async () => { const ed = await import('./editor.js?v=66fb115653'); const made = ed.insertBlock(afterId, { type: 'p' }, false); ed.setType(made.id, 'divider'); } },
    { sep: true },
    { label: 'floating text box', icon: 'textbox', hint: 'ctrl+shift+t', onPick: async () => (await import('./page.js?v=66fb115653')).addFreeBox() },
    { label: 'margin note', icon: 'sidenote', hint: 'ctrl+m', onPick: async () => (await import('./page.js?v=66fb115653')).addSidenoteFromSelection() },
    { label: 'sub-page', icon: 'cards', onPick: async () => (await import('./page.js?v=66fb115653')).addSubPage(afterId) },
    { label: 'timestamp', icon: 'clock', onPick: async () => (await import('./video.js?v=66fb115653')).insertTimestamp(afterId) },
  ];
}
...
async function imageMenu(fig, at) {
  ...
  const img = await import('./images.js?v=66fb115653');
  ...
  contextMenu([
    {
      row: [
        { icon: 'pen', tip: 'draw on it', onPick: async () => (await import('./annotate.js?v=66fb115653')).openAnnotator(id) },
        { icon: 'pin', tip: 'add a numbered pin', onPick: () => img.armPin(id) },
        { icon: 'link', tip: 'link to a line of text', onPick: async () => (await import('./anchors.js?v=66fb115653')).linkImageToLine(id) },
    ...
    { sep: true },
    { label: 'draw on it', icon: 'pen', hint: 'double-click', onPick: async () => (await import('./annotate.js?v=66fb115653')).openAnnotator(id) },
    { label: 'add a pin', icon: 'pin', onPick: () => img.armPin(id) },
    { label: 'link to a line of text', icon: 'link', onPick: async () => (await import('./anchors.js?v=66fb115653')).linkImageToLine(id) },
    ...
    { label: 'compare with another picture…', icon: 'layers',
      onPick: async () => (await import('./compare.js?v=66fb115653')).compareWith(id) },
    ...
    { sep: true },
    { label: 'delete picture', icon: 'trash', danger: true, onPick: async () => (await import('./editor.js?v=66fb115653')).deleteBlock(id) },
  ], { ...at, width: 244 });
}
...
async function pinMenu(pin, at) {
  ...
  contextMenu([
    ...
    { label: 'link it to a line of text', icon: 'link', onPick: async () => (await import('./anchors.js?v=66fb115653')).linkPinToLine(blockId, pinId) },
    { sep: true },
...
async function anchorMenu(mark, at) {
  const anchors = await import('./anchors.js?v=66fb115653');
  contextMenu([
...
async function subPageMenu(el, at) {
  ...
  const kid = card(id);
  if (!kid) return;
  const nav = await import('./nav.js?v=66fb115653');
  contextMenu([
    ...
    { label: 'rename', icon: 'pen', onPick: async () => {
      ...
      (await import('./page.js?v=66fb115653')).refreshPage();
    } },
    flagSub(id, kid),
    { sep: true },
...
async function pageBackgroundMenu(at) {
  ...
  contextMenu([
    ...
    { label: 'picture from a file', icon: 'image', onPick: async () => (await import('./images.js?v=66fb115653')).pickImageFile(lastId) },
    { label: 'sub-page', icon: 'cards', onPick: () => pg.addSubPage(null) },
    { sep: true },
    { label: 'find in this page', icon: 'find', hint: 'ctrl+h', onPick: async () => (await import('./find.js?v=66fb115653')).openFind() },
    { label: 'select everything', icon: 'grid', onPick: () => ed.selectAllBlocks() },
    { label: 'read mode', icon: 'book', hint: 'ctrl+r', onPick: async () => (await import('./readmode.js?v=66fb115653')).openReader() },
  ], { ...at, width: 226 });
}
...
async function mapNodeMenu(node, at) {
  ...
  if (!target) return;
  const nav = await import('./nav.js?v=66fb115653');
  const canvas = await import('./canvas.js?v=66fb115653');
  contextMenu([
...
async function mapBackgroundMenu(e, at) {
  ...
  contextMenu([
    ...
    { sep: true },
    { label: 'back to the page', icon: 'page', hint: 'ctrl+shift+b', onPick: async () => (await import('./nav.js?v=66fb115653')).toggleMap() },
  ], at);
}
...
async function dashMenu(at) {
  contextMenu([
    ...
    { label: 'import a session file', icon: 'upload', onPick: async () => (await import('./transfer.js?v=66fb115653')).pickSessionFile() },
    { label: 'search everything', icon: 'search', hint: 'ctrl+k', onPick: async () => (await import('./search.js?v=66fb115653')).openPalette() },
    { sep: true },
    { label: 'settings', icon: 'gear', onPick: async () => (await import('./settings.js?v=66fb115653')).openGear() },
    { label: 'open the notes folder', icon: 'folder', onPick: async () => (await import('./api.js?v=66fb115653')).api.reveal('data') },
  ], at);
}
