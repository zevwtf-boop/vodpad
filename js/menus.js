...
import { h, fmtClock, stripHtml } from './util.js?v=44ebe426f1';
import { state, card, commit, cardTitle, deleteCard, childrenOf, SEV_LABEL } from './store.js?v=44ebe426f1';
import { contextMenu, toast, promptDialog, confirmDialog } from './ui.js?v=44ebe426f1';
...
async function route(e) {
  ...
  if (shape) {
    e.preventDefault();
    const sh = await import('./shapes.js?v=44ebe426f1');
...
async function blockMenu(blk, at) {
  ...
  contextMenu([
    ...
    { sep: true },
    { label: 'note in the margin', icon: 'sidenote', hint: 'ctrl+m', onPick: async () => (await import('./page.js?v=44ebe426f1')).addSidenoteFromSelection() },
    { label: 'link this line to a picture', icon: 'link', hint: 'ctrl+l', onPick: async () => (await import('./anchors.js?v=44ebe426f1')).startAnchorPick() },
    { label: 'make it a sub-page', icon: 'cards', onPick: async () => (await import('./page.js?v=44ebe426f1')).addSubPage(id) },
    { sep: true },
    ...clipboardRows(),
    { sep: true },
...
function insertItems(afterId) {
  return [
    { label: 'picture from a file', icon: 'image', onPick: async () => (await import('./images.js?v=44ebe426f1')).pickImageFile(afterId) },
    { label: 'heading', icon: 'h2', onPick: async () => { const ed = await import('./editor.js?v=44ebe426f1'); ed.insertBlock(afterId, { type: 'h2' }, true); } },
    { label: 'bullet list', icon: 'listUl', onPick: async () => { const ed = await import('./editor.js?v=44ebe426f1'); ed.insertBlock(afterId, { type: 'ul' }, true); } },
    { label: 'checklist', icon: 'listCheck', onPick: async () => { const ed = await import('./editor.js?v=44ebe426f1'); ed.insertBlock(afterId, { type: 'todo' }, true); } },
    { label: 'quote', icon: 'quote', onPick: async () => { const ed = await import('./editor.js?v=44ebe426f1'); ed.insertBlock(afterId, { type: 'quote' }, true); } },
    { label: 'callout', icon: 'callout', onPick: async () => { const ed = await import('./editor.js?v=44ebe426f1'); ed.insertBlock(afterId, { type: 'callout' }, true); } },
    { label: 'code', icon: 'codeTag', onPick: async () => { const ed = await import('./editor.js?v=44ebe426f1'); ed.insertBlock(afterId, { type: 'code' }, true); } },
    { label: 'table', icon: 'table', onPick: async () => { const ed = await import('./editor.js?v=44ebe426f1'); const made = ed.insertBlock(afterId, { type: 'p' }, false); ed.setType(made.id, 'table', { rows: [['', '', ''], ['', '', ''], ['', '', '']], header: true }); } },
    { label: 'divider', icon: 'divider', onPick: async () => { const ed = await import('./editor.js?v=44ebe426f1'); const made = ed.insertBlock(afterId, { type: 'p' }, false); ed.setType(made.id, 'divider'); } },
    { sep: true },
    { label: 'box on the board', icon: 'roundBox', hint: 'r', onPick: async () => (await import('./shapes.js?v=44ebe426f1')).addShape() },
    { label: 'text box', icon: 'textbox', hint: 'ctrl+shift+t', onPick: async () => (await import('./shapes.js?v=44ebe426f1')).addShape({ kind: 'rect', tone: 'text', align: 'left', valign: 'top', w: 240, h: 60 }) },
    { label: 'margin note', icon: 'sidenote', hint: 'ctrl+m', onPick: async () => (await import('./page.js?v=44ebe426f1')).addSidenoteFromSelection() },
    { label: 'sub-page', icon: 'cards', onPick: async () => (await import('./page.js?v=44ebe426f1')).addSubPage(afterId) },
    { label: 'timestamp', icon: 'clock', onPick: async () => (await import('./video.js?v=44ebe426f1')).insertTimestamp(afterId) },
  ];
}
...
async function imageMenu(fig, at) {
  ...
  const img = await import('./images.js?v=44ebe426f1');
  ...
  contextMenu([
    {
      row: [
        { icon: 'pen', tip: 'draw on it', onPick: async () => (await import('./annotate.js?v=44ebe426f1')).openAnnotator(id) },
        { icon: 'pin', tip: 'add a numbered pin', onPick: () => img.armPin(id) },
        { icon: 'link', tip: 'link to a line of text', onPick: async () => (await import('./anchors.js?v=44ebe426f1')).linkImageToLine(id) },
    ...
    { sep: true },
    { label: 'draw on it', icon: 'pen', hint: 'double-click', onPick: async () => (await import('./annotate.js?v=44ebe426f1')).openAnnotator(id) },
    { label: 'add a pin', icon: 'pin', onPick: () => img.armPin(id) },
    { label: 'link to a line of text', icon: 'link', onPick: async () => (await import('./anchors.js?v=44ebe426f1')).linkImageToLine(id) },
    ...
    { label: 'compare with another picture…', icon: 'layers',
      onPick: async () => (await import('./compare.js?v=44ebe426f1')).compareWith(id) },
    { label: 'copy the picture', icon: 'copy', onPick: () => copyPicture(block) },
    { label: 'keep it as a preset', icon: 'plus', hint: 'every session',
      onPick: async () => (await import('./presets.js?v=44ebe426f1')).addPresetFromSrc(block.src, stripHtml(block.caption || '') || 'my picture') },
    ...
    { sep: true },
    { label: 'delete picture', icon: 'trash', danger: true, onPick: async () => (await import('./editor.js?v=44ebe426f1')).deleteBlock(id) },
  ], { ...at, width: 244 });
}
...
async function pinMenu(pin, at) {
  ...
  contextMenu([
    ...
    { label: 'link it to a line of text', icon: 'link', onPick: async () => (await import('./anchors.js?v=44ebe426f1')).linkPinToLine(blockId, pinId) },
    { sep: true },
...
async function anchorMenu(mark, at) {
  const anchors = await import('./anchors.js?v=44ebe426f1');
  contextMenu([
...
async function subPageMenu(el, at) {
  ...
  const kid = card(id);
  if (!kid) return;
  const nav = await import('./nav.js?v=44ebe426f1');
  contextMenu([
    ...
    { label: 'rename', icon: 'pen', onPick: async () => {
      ...
      (await import('./page.js?v=44ebe426f1')).refreshPage();
    } },
    flagSub(id, kid),
    { sep: true },
...
async function pageBackgroundMenu(at) {
  ...
  contextMenu([
    ...
    { sep: true },
    { label: 'a box here', icon: 'roundBox', hint: 'double-click', onPick: async () => (await import('./shapes.js?v=44ebe426f1')).addShapeAt(pg.toPlane(at.x, at.y)) },
    { label: 'a sticky note here', icon: 'note', hint: 'n', onPick: async () => (await import('./shapes.js?v=44ebe426f1')).addShapeAt(pg.toPlane(at.x, at.y), { kind: 'sticky', fill: '#ffd54a', tone: 'solid', align: 'left', valign: 'top', w: 180, h: 170 }) },
    { label: 'text here', icon: 'textbox', hint: 'ctrl+shift+t', onPick: async () => (await import('./shapes.js?v=44ebe426f1')).addShapeAt(pg.toPlane(at.x, at.y), { kind: 'rect', tone: 'text', align: 'left', valign: 'top', w: 240, h: 60 }) },
    { label: 'a frame here', icon: 'frame', hint: 'f', onPick: async () => (await import('./shapes.js?v=44ebe426f1')).addShapeAt(pg.toPlane(at.x, at.y), { kind: 'frame', w: 520, h: 360 }) },
    { label: 'picture from a file', icon: 'image', onPick: async () => (await import('./images.js?v=44ebe426f1')).pickImageFile(lastId) },
    ...
    { label: 'tidy the boxes into a grid', icon: 'tidy', onPick: async () => (await import('./shapes.js?v=44ebe426f1')).tidySelection() },
    { label: 'draw a line between two things', icon: 'link', hint: 'c', onPick: async () => (await import('./wires.js?v=44ebe426f1')).startWire() },
    { sep: true },
    { label: 'what this page shows', icon: 'eye', subWidth: 240, sub: () => pg.chromeItems() },
    { label: 'find in this page', icon: 'find', hint: 'ctrl+h', onPick: async () => (await import('./find.js?v=44ebe426f1')).openFind() },
...
async function mapNodeMenu(node, at) {
  ...
  if (!target) return;
  const nav = await import('./nav.js?v=44ebe426f1');
  const canvas = await import('./canvas.js?v=44ebe426f1');
  contextMenu([
...
async function mapBackgroundMenu(e, at) {
  ...
  contextMenu([
    ...
    { sep: true },
    { label: 'back to the page', icon: 'page', hint: 'ctrl+shift+b', onPick: async () => (await import('./nav.js?v=44ebe426f1')).toggleMap() },
  ], at);
}
...
async function dashMenu(at) {
  contextMenu([
    ...
    { label: 'import a session file', icon: 'upload', onPick: async () => (await import('./transfer.js?v=44ebe426f1')).pickSessionFile() },
    { label: 'search everything', icon: 'search', hint: 'ctrl+k', onPick: async () => (await import('./search.js?v=44ebe426f1')).openPalette() },
    { sep: true },
    { label: 'settings', icon: 'gear', onPick: async () => (await import('./settings.js?v=44ebe426f1')).openGear() },
    { label: 'open the notes folder', icon: 'folder', onPick: async () => (await import('./api.js?v=44ebe426f1')).api.reveal('data') },
  ], at);
}
