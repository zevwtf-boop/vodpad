...
export async function chooseBackend() {
  ...
  if (config && config.api) {
    const cloud = await import('./cloud.js?v=2e4abb3f3d');
...
export async function signOut() {
  if (mode === 'cloud') (await import('./cloud.js?v=2e4abb3f3d')).logout();
  else (await import('./vault.js?v=2e4abb3f3d')).lock();
  location.reload();
}

export async function whoAmI() {
  if (mode === 'cloud') return (await import('./cloud.js?v=2e4abb3f3d')).cloudUser();
  if (mode === 'vault') return (await import('./vault.js?v=2e4abb3f3d')).currentUser();
  return null;
}
