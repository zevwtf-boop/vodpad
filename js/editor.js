...
import { h, $, $$, uid, clamp, stripHtml, sanitizeInline, modKey, debounce } from './util.js?v=2e4abb3f3d';
import { icon } from './icons.js?v=2e4abb3f3d';
import { state, card, commit, quietly, syncTags, bus, undo } from './store.js?v=2e4abb3f3d';
import { contextMenu, toast } from './ui.js?v=2e4abb3f3d';
import { animate, EASE } from './motion.js?v=2e4abb3f3d';
import { renderImageBlock } from './images.js?v=2e4abb3f3d';
import { openSlashMenu, closeSlashMenu, slashOpen } from './toolbar.js?v=2e4abb3f3d';
...
function subPageEl(block) {
  ...
  return h('button.subpage-block', {
    on: {
      click: async (e) => {
        const { openCardPage } = await import('./nav.js?v=2e4abb3f3d');
...
async function tidyWires() {
  try { (await import('./wires.js?v=2e4abb3f3d')).pruneWires(); } catch { /* no wires module, fine */ }
}
...
async function onPaste(e, body, blockId) {
  ...
  if (imageItem) {
    ...
    const { insertImageFromFile } = await import('./images.js?v=2e4abb3f3d');
