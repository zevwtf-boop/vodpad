...
import { h, $, $$, uid, clamp, debounce } from './util.js';
import { icon } from './icons.js';
import { api, mediaUrl } from './api.js';
import { state, card, commit, quietly, bus } from './store.js';
import { toast, contextMenu, popover, closePopover, confirmDialog } from './ui.js';
import { animate, EASE, ping } from './motion.js';
...
export function renderImageBlock(block, cardId) {
  ...
  const frame = h('div.img-frame',
    ...
    h('div.img-tools',
      ...
      tool('link', 'link it to a line of text', async () => (await import('./anchors.js')).linkImageToLine(block.id)),
  ...
  popover(h('div.pin-pop',
    ...
    h('div.pin-pop-foot',
      h('button.btn.btn-sm.btn-ghost', {
        on: { click: async () => { closePopover(); (await import('./anchors.js')).linkPinToLine(blockId, pinId); } },
