...
import { h, $, $$, uid, clamp, debounce } from './util.js?v=440f02a293';
import { icon } from './icons.js?v=440f02a293';
import { api, mediaUrl } from './api.js?v=440f02a293';
import { state, card, commit, quietly, bus } from './store.js?v=440f02a293';
import { toast, contextMenu, popover, closePopover, confirmDialog } from './ui.js?v=440f02a293';
import { animate, EASE, ping } from './motion.js?v=440f02a293';
...
export function renderImageBlock(block, cardId) {
  ...
  const frame = h('div.img-frame',
    ...
    h('div.img-tools',
      ...
      tool('link', 'link it to a line of text', async () => (await import('./anchors.js?v=440f02a293')).linkImageToLine(block.id)),
  ...
  popover(h('div.pin-pop',
    ...
    h('div.pin-pop-foot',
      h('button.btn.btn-sm.btn-ghost', {
        on: { click: async () => { closePopover(); (await import('./anchors.js?v=440f02a293')).linkPinToLine(blockId, pinId); } },
...
export function refreshImage(blockId) {
  ...
  import('./editor.js?v=440f02a293').then((ed) => {
...
async function openStudio(blockId) {
  const { openAnnotator } = await import('./annotate.js?v=440f02a293');
