...
import { $, h, clamp, uid, debounce } from './util.js?v=440f02a293';
import { icon } from './icons.js?v=440f02a293';
import { state } from './store.js?v=440f02a293';
import { popIn, popOut } from './motion.js?v=440f02a293';
import { promptDialog, toast, popover, pushLayer, dropLayer } from './ui.js?v=440f02a293';
...
async function update() {
  ...
  const editor = await import('./editor.js?v=440f02a293');
...
function turnMenu(anchor, editor) {
  ...
  const list = h('div.pop-list', ...rows.map(([type, label, ico]) => h('button.menu-row', {
    on: {
      ...
      click: () => { editor.setType(id, type); import('./ui.js?v=440f02a293').then((m) => m.closePopover()); },
    },
...
function colorMenu(anchor, editor) {
  ...
  const wrap = h('div.color-pop',
    ...
    h('div.color-row', ...swatches.map(([name, css]) => h('button.color-dot', {
      ...
      on: { mousedown: keep, click: () => { editor.wrapSelection('mark', { 'data-hl': name }); import('./ui.js?v=440f02a293').then((m) => m.closePopover()); } },
    }))),
    h('div.color-label', { text: 'text' }),
    h('div.color-row', ...swatches.map(([name, css]) => h('button.color-dot.color-ring', {
      style: { color: css }, tip: name,
      on: { mousedown: keep, click: () => { editor.wrapSelection('span', { 'data-c': name }); import('./ui.js?v=440f02a293').then((m) => m.closePopover()); } },
    }, h('i', { style: { background: css } })))),
    h('button.btn.btn-sm.btn-ghost', {
      on: { mousedown: keep, click: () => { editor.clearFormatting(); import('./ui.js?v=440f02a293').then((m) => m.closePopover()); } },
    }, 'clear'),
  );
...
async function linkToPicture() {
  const { startAnchorPick } = await import('./anchors.js?v=440f02a293');
  startAnchorPick();
}

async function addSidenote() {
  const { addSidenoteFromSelection } = await import('./page.js?v=440f02a293');
...
async function pick() {
  ...
  switch (item.id) {
    ...
    case 'timestamp': {
      const { insertTimestamp } = await import('./video.js?v=440f02a293');
      insertTimestamp(blockId);
      break;
    }
    case 'subpage': {
      const { addSubPage } = await import('./page.js?v=440f02a293');
      addSubPage(blockId);
      break;
    }
    default:
