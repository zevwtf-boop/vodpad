...
import { $, h, clear, clamp, uid, download, fmtClock, parseClock } from './util.js?v=13c601f470';
import { icon } from './icons.js?v=13c601f470';
import { api } from './api.js?v=13c601f470';
import { state, commit, quietly } from './store.js?v=13c601f470';
import { toast, contextMenu, promptDialog, confirmDialog, openModal, pushLayer, dropLayer } from './ui.js?v=13c601f470';
        ...
        h('div.lm-route-name',
          ...
          route.t !== null && route.t !== undefined
            ? h('button.lm-route-t', { tip: 'jump the vod to where you ran this',
                on: { click: async () => { try { (await import('./video.js?v=13c601f470')).seekTo(route.t); } catch { /* no panel is fine */ } } },
              }, fmtClock(route.t))
            : null),
...
async function loadGhosts() {
  const { loadCorpus } = await import('./corpus.js?v=13c601f470');
...
function drawGhosts(svg, map) {
  ...
  for (const d of ghosts) {
    ...
    node.addEventListener('click', (e) => {
      ...
      shut();
      import('./nav.js?v=13c601f470').then((n) => n.go({ name: 'page', boardId: d.boardId }));
    });
    node.addEventListener('pointerdown', (e) => e.stopPropagation());
    g.append(node);
  }
