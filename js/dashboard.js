...
import { $, h, clear, fmtRel, fmtDate, previewOf, escapeHtml, debounce } from './util.js?v=2e4abb3f3d';
import { icon } from './icons.js?v=2e4abb3f3d';
import { mediaUrl, api, mode } from './api.js?v=2e4abb3f3d';
import { state, bus, createBoard, deleteBoard, refreshBoards, cardTitle, saveSettings, ownerCounts, isForeign } from './store.js?v=2e4abb3f3d';
import { registerSurface, go } from './nav.js?v=2e4abb3f3d';
import { toast, contextMenu, promptDialog, confirmDialog } from './ui.js?v=2e4abb3f3d';
import { stagger, countUp, animate, EASE } from './motion.js?v=2e4abb3f3d';
  ...
  el.append(h('header.dash-head',
    ...
    h('div.dash-actions',
      ...
      h('button.btn', { tip: 'bring in a .vodpad session file from another device or another person',
        on: { click: async () => (await import('./transfer.js?v=2e4abb3f3d')).pickSessionFile() } }, icon('upload', { size: 15 }), 'import'),
...
async function paintGrid() {
  ...
  if (view.kind === 'patterns') {
    const { paintPatterns } = await import('./patterns.js?v=2e4abb3f3d');
    return paintPatterns(body);
  }
  if (view.kind === 'accounts') {
    const { paintAccounts } = await import('./accounts.js?v=2e4abb3f3d');
...
function cardMenu(meta, anchor, x, y) {
  ...
  contextMenu([
    ...
    { sep: true },
    { label: 'export session file', icon: 'download', hint: '.vodpad', onPick: async () => (await import('./transfer.js?v=2e4abb3f3d')).exportSession(meta.id) },
    {
    ...
    { label: 'how do i send this to someone?', icon: 'link', onPick: async () => (await import('./transfer.js?v=2e4abb3f3d')).transferHelp() },
    mine ? { sep: true } : null,
...
async function clips(meta, csv) {
  try {
    ...
    const { exportClipList } = await import('./exporter.js?v=2e4abb3f3d');
