...
import { $, h, clear, fmtRel, fmtDate, previewOf, escapeHtml, debounce } from './util.js?v=7cc5d8f531';
import { icon } from './icons.js?v=7cc5d8f531';
import { mediaUrl, api, mode } from './api.js?v=7cc5d8f531';
import { state, bus, createBoard, deleteBoard, refreshBoards, cardTitle, saveSettings, ownerCounts, isForeign } from './store.js?v=7cc5d8f531';
import { registerSurface, go } from './nav.js?v=7cc5d8f531';
import { toast, contextMenu, promptDialog, confirmDialog } from './ui.js?v=7cc5d8f531';
import { stagger, countUp, animate, EASE } from './motion.js?v=7cc5d8f531';
  ...
  el.append(h('header.dash-head',
    ...
    h('div.dash-actions',
      ...
      h('button.btn', { tip: 'bring in a .vodpad session file from another device or another person',
        on: { click: async () => (await import('./transfer.js?v=7cc5d8f531')).pickSessionFile() } }, icon('upload', { size: 15 }), 'import'),
...
async function paintGrid() {
  ...
  if (view.kind === 'patterns') {
    const { paintPatterns } = await import('./patterns.js?v=7cc5d8f531');
    return paintPatterns(body);
  }
  if (view.kind === 'accounts') {
    const { paintAccounts } = await import('./accounts.js?v=7cc5d8f531');
...
function cardMenu(meta, anchor, x, y) {
  ...
  contextMenu([
    ...
    { sep: true },
    { label: 'export session file', icon: 'download', hint: '.vodpad', onPick: async () => (await import('./transfer.js?v=7cc5d8f531')).exportSession(meta.id) },
    {
    ...
    { label: 'how do i send this to someone?', icon: 'link', onPick: async () => (await import('./transfer.js?v=7cc5d8f531')).transferHelp() },
    mine ? { sep: true } : null,
...
async function clips(meta, csv) {
  try {
    ...
    const { exportClipList } = await import('./exporter.js?v=7cc5d8f531');
...
async function newSession() {
  ...
  // NOTE: not `blocks?.length` — "blank" has no blocks and its whole job is to
  // empty the page and turn the paper off, so it has to be applied too.
  if (picked.template) {
    try {
