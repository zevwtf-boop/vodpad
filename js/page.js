...
import { $, $$, h, clear, uid, clamp, debounce, rafThrottle, fmtClock, stripHtml } from './util.js?v=7cc5d8f531';
import { icon } from './icons.js?v=7cc5d8f531';
import { mediaUrl } from './api.js?v=7cc5d8f531';
import { stampsOnCard, tidy, clipLines } from './stamps.js?v=7cc5d8f531';
import { state, card, commit, quietly, bus, cardTitle, childrenOf, makeCard, deleteCard, syncTags, allTags, setSetting, SEV_LABEL, SEV_ORDER } from './store.js?v=7cc5d8f531';
import { registerSurface, go, openCardPage, toggleMap } from './nav.js?v=7cc5d8f531';
import { renderBlocks, insertBlock, currentBlockId, currentBody, focusBlock, getBlock, blockElById, pageStats } from './editor.js?v=7cc5d8f531';
import { initSelectionToolbar } from './toolbar.js?v=7cc5d8f531';
import { contextMenu, toast, popover, promptDialog, confirmDialog } from './ui.js?v=7cc5d8f531';
import { stagger, animate, ping, EASE } from './motion.js?v=7cc5d8f531';
import { paintAnchors } from './anchors.js?v=7cc5d8f531';
import { paintWires, startWire, cancelWire, wiring } from './wires.js?v=7cc5d8f531';
import { mountVideoPanel } from './video.js?v=7cc5d8f531';
import { mountWhiteboard, activeTool, renderInk, paintZoomPill } from './whiteboard.js?v=7cc5d8f531';
import { renderShapes, startMarquee, addShapeAt, addShape, clearShapeSelection, tidySelection, shapesOf, syncZoom, selectShape } from './shapes.js?v=7cc5d8f531';
...
export function fitBoard() {
  ...
  // ink is on the plane too, and on a page that is mostly drawing it is the
  // only thing there — "fit everything" has to mean everything
  for (const stroke of c?.ink || []) {
    const pts = stroke.pts || [[stroke.x, stroke.y], [stroke.x2, stroke.y2]];
  ...
  if (!boxes.length && !onPlane && !(c?.ink || []).length) { resetPageView(); return; }
  const pad = 46;
...
export function render() {
  ...
  // Element.append(null) renders the literal text "null" — the hint is only
  // there on a bare page, so it has to be filtered out rather than passed
  work.append(...[viewport, emptyPlaneHint(c, chrome), statusBar()].filter(Boolean));
  ...
  for (const s of [...frames, ...boxes]) {
    ...
    list.append(h('button.side-row', {
      ...
      on: {
        ...
        contextmenu: (e) => {
          e.preventDefault();
          import('./shapes.js?v=7cc5d8f531').then((m) => m.shapeMenu(s.id, e.clientX, e.clientY));
        },
      },
    },
...
function paintImages(body, c) {
  ...
  if (!shots.length && !boxed.length) {
    body.append(h('div.side-empty',
      ...
      h('button.btn.btn-sm', { on: { click: async () => (await import('./images.js?v=7cc5d8f531')).pickImageFile(card(state.cardId).blocks.at(-1)?.id) } },
...
async function dropPresetHere(preset) {
  const { dropPreset } = await import('./presets.js?v=7cc5d8f531');
...
function paintPresets(body) {
  import('./presets.js?v=7cc5d8f531').then((lib) => {
...
function paintTimeline(body, c) {
  ...
  if (!rows.length) {
    body.append(h('div.side-empty',
      ...
      h('button.btn.btn-sm', { on: { click: async () => (await import('./video.js?v=7cc5d8f531')).insertTimestamp(currentBlockId() || c.blocks.at(-1)?.id) } },
...
async function goToStamp(row) {
  try { (await import('./video.js?v=7cc5d8f531')).seekTo(row.t); } catch { /* no video panel is fine */ }
...
function paintAddPalette(body, c) {
  ...
  const items = [
    ...
    ['picture', 'image', async () => (await import('./images.js?v=7cc5d8f531')).pickImageFile(at())],
    ...
    ['timestamp', 'clock', async () => (await import('./video.js?v=7cc5d8f531')).insertTimestamp(at())],
  ];
...
function paintMeta() {
  ...
  if (c.t !== null && c.t !== undefined) {
    wrap.append(h('button.chip.chip-time', {
      ...
      on: { click: async () => (await import('./video.js?v=7cc5d8f531')).seekTo(c.t) },
...
function pageMenu(anchor) {
  ...
  contextMenu([
    ...
    { label: 'arrange this page…', icon: 'grid', hint: 'presets',
      onPick: async () => (await import('./layouts.js?v=7cc5d8f531')).openLayouts() },
    { sep: true },
    { label: 'loot routes', icon: 'target', hint: 'map',
      onPick: async () => (await import('./lootmap.js?v=7cc5d8f531')).openLootmap() },
    { label: 'earlier versions of this session', icon: 'history',
      onPick: async () => (await import('./history.js?v=7cc5d8f531')).openHistory() },
    { label: 'save this page as a template', icon: 'copy', hint: 'reuse',
      onPick: async () => (await import('./templates.js?v=7cc5d8f531')).saveAsTemplate(c) },
    { sep: true },
    { label: 'export markdown', icon: 'download', onPick: async () => (await import('./exporter.js?v=7cc5d8f531')).exportMarkdown(c.id) },
    { label: 'export html', icon: 'download', onPick: async () => (await import('./exporter.js?v=7cc5d8f531')).exportHtml(c.id) },
    { label: 'export the whole session (.vodpad)', icon: 'download', onPick: async () => (await import('./transfer.js?v=7cc5d8f531')).exportSession(state.board.id) },
    { label: 'export the clip list', icon: 'clock', hint: '.csv',
      onPick: async () => (await import('./exporter.js?v=7cc5d8f531')).exportClipList(state.board, { csv: true }) },
...
export function addSidenoteFromSelection() {
  ...
  if (sel && !sel.isCollapsed) {
    import('./editor.js?v=7cc5d8f531').then((ed) => ed.wrapSelection('span', { 'data-side': noteId }));
  } else {
...
function freeBoxEl(box) {
  ...
  if (sticky) {
    el.addEventListener('contextmenu', (e) => {
      e.preventDefault();
      e.stopPropagation();
      import('./whiteboard.js?v=7cc5d8f531').then(({ STICKY_COLOURS }) => {
        contextMenu([
...
bus.on('shapes:changed', () => paintStatus());
