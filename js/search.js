...
import { $, h, clear, debounce, fuzzy, highlight, stripHtml, previewOf, fmtRel } from './util.js?v=5aab9d9b3f';
import { icon } from './icons.js?v=5aab9d9b3f';
import { state, card, cardTitle, createBoard } from './store.js?v=5aab9d9b3f';
import { pushLayer, dropLayer, toast } from './ui.js?v=5aab9d9b3f';
import { popIn, popOut } from './motion.js?v=5aab9d9b3f';
import { go, toggleMap } from './nav.js?v=5aab9d9b3f';
import { allCards } from './corpus.js?v=5aab9d9b3f';
import { openGear } from './settings.js?v=5aab9d9b3f';
import { showView } from './dashboard.js?v=5aab9d9b3f';
  ...
  return [
    ...
    { id: 'new', title: 'new session', sub: 'start a fresh vod review', ico: 'plus', hint: 'n', run: async () => {
      ...
      if (picked.template?.blocks?.length) {
        ...
        const { api } = await import('./api.js?v=5aab9d9b3f');
    ...
    inBoard && { id: 'read', title: 'read mode', sub: 'the whole branch as one document', ico: 'book', hint: 'ctrl+r', run: async () => (await import('./readmode.js?v=5aab9d9b3f')).openReader() },
    inBoard && { id: 'sub', title: 'add a sub-page', sub: 'expand on the topic you are in', ico: 'cards', run: async () => (await import('./page.js?v=5aab9d9b3f')).addSubPage(null) },
    inBoard && { id: 'shape', title: 'put a box on the board', sub: 'a filled box you can join to others', ico: 'roundBox', hint: 'r', run: async () => (await import('./shapes.js?v=5aab9d9b3f')).addShape() },
    inBoard && { id: 'box', title: 'text box', sub: 'drop text anywhere on the plane', ico: 'textbox', hint: 'ctrl+shift+t', run: async () => (await import('./shapes.js?v=5aab9d9b3f')).addShape({ kind: 'rect', tone: 'text', align: 'left', valign: 'top', w: 240, h: 60 }) },
    inBoard && { id: 'frame', title: 'frame a part of the board', sub: 'a titled area that carries what is inside it', ico: 'frame', run: async () => (await import('./shapes.js?v=5aab9d9b3f')).addShape({ kind: 'frame', w: 520, h: 360 }) },
    inBoard && { id: 'tidy', title: 'tidy the boxes into a grid', sub: 'everything on the board, evenly spaced', ico: 'tidy', run: async () => (await import('./shapes.js?v=5aab9d9b3f')).tidySelection() },
    inBoard && { id: 'chrome', title: 'what this page shows', sub: 'the paper, the title, the flags row, the sub-pages — turn any of them off', ico: 'eye',
      run: async () => {
        ...
        const { contextMenu } = await import('./ui.js?v=5aab9d9b3f');
    ...
    inBoard && { id: 'preset', title: 'drop-on pictures', sub: 'the markers, arrows and build pieces you keep drawing', ico: 'image',
      run: async () => { const pg = await import('./page.js?v=5aab9d9b3f'); pg.pickSideTab('shots'); } },
    ...
    inBoard && { id: 'video', title: 'video panel', sub: 'a recording or a youtube link', ico: 'video', hint: 'v', run: async () => (await import('./video.js?v=5aab9d9b3f')).toggleVideo() },
    inBoard && { id: 'frames', title: 'pick a frame', sub: 'the eight either side of where the vod is', ico: 'layers', hint: 'shift+s', run: async () => (await import('./video.js?v=5aab9d9b3f')).pickFrame() },
    inBoard && { id: 'arrange', title: 'arrange this page', sub: 'notes beside the picture, a grid of shots, or spread out to draw on', ico: 'grid', run: async () => (await import('./layouts.js?v=5aab9d9b3f')).openLayouts() },
    inBoard && { id: 'wire', title: 'draw a line between two things', sub: 'click one, click the other', ico: 'link', hint: 'c', run: async () => (await import('./wires.js?v=5aab9d9b3f')).startWire() },
    inBoard && { id: 'loot', title: 'loot routes', sub: 'plan a drop and a rotation on the real island', ico: 'target', run: async () => (await import('./lootmap.js?v=5aab9d9b3f')).openLootmap() },
    inBoard && { id: 'history', title: 'earlier versions of this session', sub: 'look at one, or go back to it', ico: 'history', run: async () => (await import('./history.js?v=5aab9d9b3f')).openHistory() },
    inBoard && { id: 'tpl', title: 'save this page as a template', sub: 'start future sessions from it', ico: 'copy', run: async () => {
      const { saveAsTemplate } = await import('./templates.js?v=5aab9d9b3f');
      const { card } = await import('./store.js?v=5aab9d9b3f');
      saveAsTemplate(card(state.cardId));
    } },
    inBoard && { id: 'md', title: 'export markdown', sub: 'this page and everything under it', ico: 'download', run: async () => (await import('./exporter.js?v=5aab9d9b3f')).exportMarkdown(state.cardId) },
    inBoard && { id: 'html', title: 'export html', sub: 'self-contained, pictures included', ico: 'download', run: async () => (await import('./exporter.js?v=5aab9d9b3f')).exportHtml(state.cardId) },
    inBoard && { id: 'clips', title: 'export the clip list', sub: 'every timestamp in the session, for an editor', ico: 'clock', hint: '.csv', run: async () => (await import('./exporter.js?v=5aab9d9b3f')).exportClipList(state.board, { csv: true }) },
    ...
    { id: 'files', title: 'open the notes folder', sub: 'your json + png files on disk', ico: 'folder', run: async () => (await import('./api.js?v=5aab9d9b3f')).api.reveal('data') },
  ].filter(Boolean);
}
