...
import { api, isStatic, mode } from './api.js?v=44ebe426f1';
import { state, setSetting, bus } from './store.js?v=44ebe426f1';
import { pushLayer, dropLayer, labelled, segmented, toast, confirmDialog } from './ui.js?v=44ebe426f1';
import { animate, settle, fadeOut, EASE } from './motion.js?v=44ebe426f1';
...
function surfaceTab(s) {
  ...
  const paintRestore = () => {
    clear(restore);
    Promise.all([import('./presets.js?v=44ebe426f1'), import('./templates.js?v=44ebe426f1')]).then(([presets, templates]) => {
  ...
  return h('div',
    ...
    state.board ? h('section.gear-section',
      ...
      h('div', { style: { display: 'flex', gap: '8px', flexWrap: 'wrap' } },
        h('button.btn', { on: { click: async () => (await import('./exporter.js?v=44ebe426f1')).exportMarkdown(state.board.rootId) } }, icon('download', { size: 14 }), 'markdown'),
        h('button.btn', { on: { click: async () => (await import('./exporter.js?v=44ebe426f1')).exportHtml(state.board.rootId) } }, icon('download', { size: 14 }), 'html'),
        h('button.btn', { on: { click: async () => (await import('./readmode.js?v=44ebe426f1')).openReader({ print: true }) } }, icon('page', { size: 14 }), 'pdf / print')),
    ) : null,
    h('section.gear-section',
      h('h3', { text: 'server' }),
...
function webDataTab() {
  ...
  if (!synced) {
    import('./vault.js?v=44ebe426f1').then(async (v) => {
  ...
  return h('div',
    h('section.gear-section',
      ...
      size,
      h('button.btn', { style: { marginTop: '12px' }, on: { click: async () => (await import('./api.js?v=44ebe426f1')).signOut() } },
...
async function doBackup() {
  const v = await import('./vault.js?v=44ebe426f1');
  const { download } = await import('./util.js?v=44ebe426f1');
