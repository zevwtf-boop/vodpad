  ...
  return h('div',
    ...
    state.board ? h('section.gear-section',
      ...
      h('div', { style: { display: 'flex', gap: '8px', flexWrap: 'wrap' } },
        h('button.btn', { on: { click: async () => (await import('./exporter.js')).exportMarkdown(state.board.rootId) } }, icon('download', { size: 14 }), 'markdown'),
        h('button.btn', { on: { click: async () => (await import('./exporter.js')).exportHtml(state.board.rootId) } }, icon('download', { size: 14 }), 'html'),
        h('button.btn', { on: { click: async () => (await import('./readmode.js')).openReader({ print: true }) } }, icon('page', { size: 14 }), 'pdf / print')),
    ) : null,
    h('section.gear-section',
      h('h3', { text: 'server' }),
