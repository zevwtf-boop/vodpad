...
import { api, isStatic, mode } from './api.js?v=d258d51ea6';
import { state, setSetting, bus } from './store.js?v=d258d51ea6';
import { pushLayer, dropLayer, labelled, segmented, toast, confirmDialog } from './ui.js?v=d258d51ea6';
import { animate, settle, fadeOut, EASE } from './motion.js?v=d258d51ea6';
...
function surfaceTab(s) {
  ...
  const paintRestore = () => {
    clear(restore);
    Promise.all([import('./presets.js?v=d258d51ea6'), import('./templates.js?v=d258d51ea6')]).then(([presets, templates]) => {
  ...
  return h('div',
    ...
    state.board ? h('section.gear-section',
      ...
      h('div', { style: { display: 'flex', gap: '8px', flexWrap: 'wrap' } },
        h('button.btn', { on: { click: async () => (await import('./exporter.js?v=d258d51ea6')).exportMarkdown(state.board.rootId) } }, icon('download', { size: 14 }), 'markdown'),
        h('button.btn', { on: { click: async () => (await import('./exporter.js?v=d258d51ea6')).exportHtml(state.board.rootId) } }, icon('download', { size: 14 }), 'html'),
        h('button.btn', { on: { click: async () => (await import('./readmode.js?v=d258d51ea6')).openReader({ print: true }) } }, icon('page', { size: 14 }), 'pdf / print')),
    ) : null,
    h('section.gear-section',
      h('h3', { text: 'server' }),
...
function webDataTab() {
  ...
  if (!synced) {
    import('./vault.js?v=d258d51ea6').then(async (v) => {
  ...
  return h('div',
    h('section.gear-section',
      ...
      size,
      h('button.btn', { style: { marginTop: '12px' }, on: { click: async () => (await import('./api.js?v=d258d51ea6')).signOut() } },
...
async function doBackup() {
  const v = await import('./vault.js?v=d258d51ea6');
  const { download } = await import('./util.js?v=d258d51ea6');
